...
import { PostDistributionDataSource } from 'src/apiAndObjects/objects/postDistributionDataSource';
...
> {
  ...
  protected callLive(body: SaveDistributionBody): Promise<PostDistributionDataSource> {
    ...
    return this.buildObjectFromResponse(PostDistributionDataSource, callResponsePromise).then(
      (response: PostDistributionDataSource) => response,
    );
  }

  protected callMock(): Promise<PostDistributionDataSource> {
